...
namespace imady.NebuUI.Samples
{
    ...
    /// 负责摄像机的调整动作
    /// </summary>
    public class NbuCameraManager : NebuEventUnityObjectBase,
        INebuProvider<CameraScrollMsg>,//相机缩放倍数消息，通知物体对象反向缩放
        ...
        INebuObserver<NebuUnityUIMessage<MouseDragMsg>>
    {
        ...
        public float m_MouseDragFactor => AppConfiguration.defaultMouseDragRatio * mainCamera.orthographicSize / m_DefaultMainCameraFOV;
        public float m_MouseScrollFactor => AppConfiguration.defualtMouseScrollRatio;
        ...
        public void OnNext(NebuUnityUIMessage<MouseDragMsg> message)
        {
            ...
            if (m_earthDisplayMode == "3D" && ((message.messageBody.xDragDeltaRatio + message.messageBody.yDragDeltaRatio) != 0 || message.messageBody.scrollRatio != 0))
            {
                ...
                m_currentCameraFOV3D = Mathf.Clamp(
                    mainCamera.fieldOfView * (1 - (message.messageBody.scrollRatio * AppConfiguration.defualtMouseScrollRatio)),
                    2, 80);
                SetCamera3D();
                //通知需要反向缩放的物体
            ...
            if (m_earthDisplayMode == "2D" && ((message.messageBody.xDragDeltaRatio + message.messageBody.yDragDeltaRatio) != 0 || message.messageBody.scrollRatio != 0))
            {
                ...
                m_currentCameraFOV2D = Mathf.Clamp(
                    mainCamera.orthographicSize * (1 - (message.messageBody.scrollRatio * AppConfiguration.defualtMouseScrollRatio)),
                    2, 40);
                SetCamera2D();
