...
{
    public abstract class NebuUIViewBase : NebuEventUnityObjectBase, INebuUIView
    {
        ...
        /// </summary>
        //protected NbuUIManager m_manager;
        ...
        /// </summary>
        /// <param name="template">也可以默认以transform.gameObject作为template（需要提前将MdyViewBase子类脚本挂到模板prefab上）</param>
        public virtual INebuUIView SetTemplate(GameObject template)
        {
            ViewTemplate = template;
            return this as INebuUIView;
        }
        public virtual INebuUIView SetTemplate()
        {
        ...
        ///// 冒泡方式太复杂了，不好 ~~~ 
        ///// </summary>
        //public virtual INbuUIView SetManager(NbuUIManager manager)
        //{
        ...
        protected virtual GameObject RenderView(GameObject template, object viewmodel)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                try
                {
                    ...
                    //}
                    //Debug.Log($"[MdyViewBase视图模板] 渲染：{property.Name}={property.GetValue(source)}");
                }
                catch (Exception e)
                {
        ...
        protected virtual GameObject RenderView_TMPText(GameObject template, object viewmodel)
        {
            ...
            foreach (PropertyInfo property in properties)
            {
                ...
                {
                    BindingText(template, property.Name, property.GetValue(viewmodel).ToString());
                    Debug.Log($"[MdyViewBase视图模板] 渲染：{property.Name}={property.GetValue(viewmodel)}");
                }
                catch (Exception e)
                {
